    ...
    <section id="etkinlik-turleri" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {types.map(({ emoji, title, desc }, i) => (
            <Reveal key={title} delay={(i % 3) * 100}>
              <div className="group relative h-full p-8 rounded-3xl border border-slate-100 bg-gradient-to-br from-white to-slate-50 hover:from-brand-50 hover:to-rose-50 hover:border-brand-100 transition-all duration-300 hover:shadow-xl hover:-translate-y-1.5">
                <div className="text-5xl mb-4 group-hover:scale-125 group-hover:-rotate-6 transition-transform duration-300 inline-block">{emoji}</div>
                <h3 className="text-xl font-bold text-slate-900 mb-2">{title}</h3>
                <p className="text-slate-500 text-sm leading-relaxed">{desc}</p>
                <div className="absolute bottom-6 right-6 w-8 h-8 rounded-full bg-gradient-to-br from-brand-500 to-rose-500 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0 flex items-center justify-center">
